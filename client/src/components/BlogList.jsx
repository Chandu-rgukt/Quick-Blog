...
function BlogList() {
    ...
    const filteredBlogs = ()=>{
        ...
        return blog.filter((blog)=>blog.title.toLowerCase().includes(input.toLowerCase())
                ...
                <div key={item} className='relative'>
                    <button onClick={()=>setMenu(item)}
                     className={`cursor-pointer text-gray-500 ${menu===item&& 'text-white px-4 pt-0.5'}`}>
                        ...
                        transition={{type: "spring", stiffness: 500,damping:30}} className='absolute left-0 right-0 top-0 h-7 -z-10
